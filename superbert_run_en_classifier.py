...
def qa_convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if is_training and example.is_impossible:
            ...
            tok_end_position = -1
        if is_training and not example.is_impossible:# 将原来的token使用tokenizer进行分词，然后对分词后的start_position和end_position进行调整
        ...
        while start_offset < len(all_doc_tokens):
            ...
            if start_offset + length == len(all_doc_tokens):
                break
            start_offset += min(length, doc_stride) #length和doc_stride不一定哪个小，因为length收到max_tokens_for_doc的影响
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                is_max_context = _check_is_max_context(doc_spans, doc_span_index,
                                                       split_token_index)
                token_is_max_context[len(tokens)] = is_max_context# dev数据集eval时，如果为False，认为在当前slice里，该token没有成为start的资格
            ...
            if example_index%5000==0: 
                print("convert to features: %{}".format(100*((example_index+1)*1.0/len(examples))))
            features.append(
                QAInputFeatures(
                    unique_id=unique_id,
                    ...
                    tokens=tokens,
                    token_to_orig_map=token_to_orig_map,# 构成answer的token在tokens的index到原来context的index的映射
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for c in paragraph_text:
                ...
                char_to_word_offset.append(len(doc_tokens) - 1)# 得到每个字符对应第几个token
            ...
            for qa in paragraph["qas"]:# 处理qas
                ...
                if is_training:
                    ...
                    if (len(qa["answers"]) != 1) and (not is_impossible):# 2.0 is_impossible=True，表示问题不可回答，此时qa["answers"]=0
                        raise ValueError(
                    ...
                    if not is_impossible:
                        ...
                        start_position = char_to_word_offset[answer_offset]# 找到答案开始于第几个token
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name == "cola":
        return {"mcc": matthews_corrcoef(labels, preds)}#马修斯相关性系数
    elif task_name == "sst-2":
        return {"acc": simple_accuracy(preds, labels)}
    elif task_name == "mrpc":
        return acc_and_f1(preds, labels)#mrpc 样本类别不均衡，故取acc和f1的均值
    elif task_name == "sts-b":
        return pearson_and_spearman(preds, labels)#sts-b 是个回归任务，但是可以使用分类来做
    elif task_name == "qqp":
        return acc_and_f1(preds, labels)#qqp 样本类别不均衡，故取acc和f1的均值
    elif task_name == "mnli":
        return {"acc": simple_accuracy(preds, labels)}
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if version_2_with_negative:
                ...
                if feature_null_score < score_null:
                    ...
                    null_start_logit = result.start_logits[0]# squad2训练时空答案start=0,end=0,因此将position 0的start+end的logits作为空答案的目标
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    if end_index not in feature.token_to_orig_map:
                        continue
                    if not feature.token_is_max_context.get(start_index, False):# start_index不能对应token_is_max_context==False
                        continue
...
def main():
    ...
    parser.add_argument("--model",
                        default='model/original_pretrained_model',
                        type=str,
                        required=False,
    ...
    parser.add_argument("--output_dir",
                        default='model/fine-tuned_pretrained_model/squad1',
                        type=str,
                        required=False,
    ...
    # fine-tuning default parameters # ？ squad2的参数
    default_params = {
    ...
    for seed in seeds:
        ...
        config = BertConfig.from_pretrained(
            args.model, num_labels=2, finetuning_task='sst-2')
        ...
        if not args.arches_file:
            ...
            subbert_config['sample_intermediate_sizes'] = [
                config.intermediate_size] * config.num_hidden_layers
            subbert_config['sample_num_attention_heads'] = [
                config.num_attention_heads] * config.num_hidden_layers
            try:
                subbert_config['sample_qkv_sizes'] = [
                    config.qkv_size] * config.num_hidden_layers
            except:
                subbert_config['sample_qkv_sizes'] = [
        ...
        for subbert_config in subbert_configs:
            logger.info(
                'The current subbert_config is: {}'.format(subbert_config))
            for task_name, data_dir in zip(task_lis, data_dir_lis):
                ...
                if output_mode != 'qa_classification':  # ！这里可以修改为get_aug_examples
                ...
                if output_mode == 'qa_classification':
                    model = SuperBertForQuestionAnswering.from_pretrained(
                        args.model, config)
                else:
                ...
                for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
                    ...
                    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                        ...
                        if output_mode == 'qa_classification':
                            ...
                            if output_mode == "classification":
                                cls_loss = loss_fct(
                                    student_logits.view(-1, num_labels), label_ids.view(-1))
                            elif output_mode == "regression":
                                cls_loss = loss_mse(
                                    student_logits.view(-1), label_ids.view(-1))

                        loss = cls_loss # cls_loss是分类的真实loss
                        ...
                        if n_gpu > 1:
                            ...
                            loss = loss.mean()
                        if args.gradient_accumulation_steps > 1:
                            loss = loss / args.gradient_accumulation_steps # 累积到相应步数之后进行参数更新，这里的loss是用于更新的loss，当gradient_accumulation_steps=1时，这里的loss和cls_loss相同
                        ...
                        if (global_step + 1) % args.eval_step == 0:
                            ...
                            model.train()
                # mnli数据集有2中dev测试数据集，dev_matched.tsv和dev_mismatched.tsv，这里只是单独在dev_mismatched.tsv上进行测试
                if task_name == "mnli":
                    task_name = "mnli-mm"
                    processor = processors[task_name]()
                    ...
                    # if oncloud:
                    #     mox.file.copy(tmp_output_eval_file, os.path.join(
                    #         args.train_url, "mm-eval_results.txt"))
                    task_name = "mnli"
                ...
                output_eval_file = os.path.join(
                    args.output_dir, "subbert.results")
                with open(output_eval_file, "a+") as writer:
                    writer.write(output_str + '\n')

            # if oncloud:
            #     # add 3, for huawei yun.
            #     logging.info(mox.file.list_directory(
            ...
            #     mox.file.copy(output_eval_file, os.path.join(
            #         args.train_url, "log.txt"))
...
if __name__ == "__main__":
    ...
    logger.info("Task cost {} minutes, i.e. {} hours. ".format((datetime.now()-start0).total_seconds()/60,(datetime.now()-start0).total_seconds()/3600))