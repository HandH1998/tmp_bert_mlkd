#!/bin/bash
list_task_name=("cola" "mrpc" "rte" "sst-2" "sts-b" "qqp" "qnli" "mnli")
list_data_name=("CoLA" "MRPC" "RTE" "SST-2" "STS-B" "QQP" "QNLI" "MNLI")
# distill the intermediate layer with resual knowledge review with enhanced respective fusion
base_dir1="model/knowledge_review/distilled_intermediate_model/resual_kr_with_multi_teacher_layer_v3"
base_dir2="model/knowledge_review/distilled_prediction_model/resual_kr_with_multi_teacher_layer_v3"
num=0
...
do
    ...
    --do_lower_case \
    --tensorboard_log_save_dir "tensorboard_log/distilled_intermediate_model/resual_kr_with_multi_teacher_layer_v3/${list_task_name[${num}]}/on_original_data" \
    --gradient_accumulation_steps 1 \
    > "log/distilled_intermediate_model/resual_kr_with_multi_teacher_layer_v3/${list_task_name[${num}]}.log" 2>&1 
    ...
    --do_lower_case \
    --tensorboard_log_save_dir "tensorboard_log/distilled_prediction_model/resual_kr_with_multi_teacher_layer_v3/${list_task_name[${num}]}/on_original_data" \
    --gradient_accumulation_steps 1 \
    > "log/distilled_prediction_model/final_resual_kr_with_multi_teacher_layer_v3/${list_task_name[${num}]}.log" 2>&1 
    let "num++"
done
