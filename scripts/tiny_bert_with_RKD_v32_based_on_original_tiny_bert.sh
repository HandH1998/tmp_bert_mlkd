#!/bin/bash
list_task_name=("cola" "mrpc" "rte" "sst-2" "qnli" "mnli" "qqp" "sts-b")
list_data_name=("CoLA" "MRPC" "RTE" "SST-2" "QNLI" "MNLI" "QQP" "STS-B")
# distill the intermediate layer with resual knowledge review with enhanced respective fusion
base_dir1="model/distilled_fine-tuned_model"
base_dir2="model/multi_level_distillation/distilled_prediction_model/tiny_bert_with_RKD_v32_based_on_original_tiny_bert"
num=0
...
do
    ...
    --do_lower_case \
    --tensorboard_log_save_dir "tensorboard_log/multi_level_distillation/distilled_prediction_model/tiny_bert_with_RKD_v32_based_on_original_tiny_bert/${list_task_name[${num}]}/on_original_data" \
    --gradient_accumulation_steps 1 \
    > "log/multi_level_distillation/distilled_prediction_model/tiny_bert_with_RKD_v32_based_on_original_tiny_bert/${list_task_name[${num}]}.log" 2>&1 
    let "num++"
done
