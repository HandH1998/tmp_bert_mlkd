#!/bin/bash
list_task_name=("mnli" "mrpc" "qqp" "sts-b")
list_data_name=("MNLI" "MRPC" "QQP" "STS-B")
# distill the intermediate layer with resual knowledge review with enhanced respective fusion
base_dir="model/distilled_fine-tuned_model"
num=0
...
do
    ...
    --do_lower_case \
    --tensorboard_log_save_dir "tensorboard_log/distilled_intermediate_model/tiny_bert/${list_task_name[${num}]}/on_original_data" \
    --gradient_accumulation_steps 1 \
    > "tiny_bert/${list_task_name[${num}]}.log" 2>&1 
    ...
    --data_dir "data/glue_data/${list_data_name[${num}]}" \
    --task_name "${list_task_name[${num}]}" \
    --output_dir "${base_dir}/${list_task_name[${num}]}/on_original_data_final"  \
    --do_lower_case \
    --tensorboard_log_save_dir "tensorboard_log/distilled_prediction_model/tiny_bert/${list_task_name[${num}]}/on_original_data" \
    --gradient_accumulation_steps 1 \
    > "final_tiny_bert/${list_task_name[${num}]}.log" 2>&1 
    let "num++"
done
