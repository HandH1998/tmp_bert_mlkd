...
from .file_utils import WEIGHTS_NAME, CONFIG_NAME, FINAL_WEIGHTS_NAME
...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if any(n in ["adam_v", "adam_m", "global_step", "seq_relationship"] for n in name):
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        is_student_final = kwargs.get('is_student_final', False)
        kwargs.pop('is_student_final', None)
        ...
        if state_dict is None and not from_tf:
            ...
            if is_student_final:
                weight_name = FINAL_WEIGHTS_NAME
...
class TinyBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, fit_size=768):
        ...
        # self.share_conv = nn.Conv2d(in_channels=1,out_channels=fit_size, kernel_size=(1, config.hidden_size))
        # self.conv = nn.ModuleList([nn.Conv2d(in_channels=1,out_channels=fit_size, kernel_size=(1, config.hidden_size))
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None, is_student=False, is_conv=False, share_param=True):
        ...
        if is_student:
            for s_id, sequence_layer in enumerate(sequence_output):
                # if is_conv:
                #     sequence_layer = torch.unsqueeze(sequence_layer, 1)
                #     if share_param:
                #         conv_sequence_layer = self.share_conv(sequence_layer)
                #     else:
                #         conv_sequence_layer = self.conv[s_id](sequence_layer)
                #     conv_sequence_layer = torch.squeeze(conv_sequence_layer, 3)
                #     conv_sequence_layer = conv_sequence_layer.permute(0,2,1)
                #     tmp.append(conv_sequence_layer)
                # else:
                if share_param:
