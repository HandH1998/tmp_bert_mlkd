...
def prepare_embedding_retrieval(glove_file, vocab_size=100000):
    ...
    with open(glove_file, 'r', encoding='utf-8') as fin:
        for line in fin:
            ...
            # if items[0]=='.': count +=1
            # embeddings[items[0]] = [float(x) if x != '.' else 0.00 for x in items[1:]]
...
def main():
    ...
    parser.add_argument("--pretrained_bert_model", default='model/original_pretrained_model', type=str, required=False,
                        help="Downloaded pretrained model (bert-base-cased/uncased) is under this folder")
    parser.add_argument("--glove_embs", default='data/glove/glove.840B.300d.txt', type=str, required=False,
                        help="Glove word embeddings file")
    parser.add_argument("--glue_dir", default='data/glue_data', type=str, required=False,
                        help="GLUE data dir")
