    ...
    def _sample_parameters(self, in_index=None, out_index=None):
        ...
        if self.bias is not None:
            self.samples['bias'] = sample_bias(
                self.bias, self.sample_out_dim, out_index=out_index)
        return self.samples
...
def sample_weight(weight, sample_in_dim, sample_out_dim, in_index=None, out_index=None):
    ...
    if out_index is None:
        ...
        sample_weight = sample_weight.index_select(
            0, out_index.to(sample_weight.device))
...
class SuperEmbedding(nn.Module):
    def __init__(self, dict_size, embd_size, padding_idx=None):
        ...
        self.embedding = nn.Embedding(
            dict_size, embd_size, padding_idx=padding_idx)
        self.sample_embedding_weight = None

    def set_sample_config(self, sample_embed_dim):# ! ...是取前边所有的维度
        self.sample_embedding_weight = self.embedding.weight[...,
                                                             :sample_embed_dim]
...
class SuperBertEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.word_embeddings = SuperEmbedding(
            config.vocab_size, config.hidden_size, padding_idx=0)  # ！vocab的第0个是[PAD]，其嵌入始终为0，也不进行梯度更新
        self.position_embeddings = SuperEmbedding(
            config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = SuperEmbedding(
            config.type_vocab_size, config.hidden_size)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = SuperBertLayerNorm(
...
class SuperBertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        new_x_shape = x.size()[#这里是拼接维度
            :-1] + (self.sample_num_attention_head, self.sample_attention_head_size)
    ...
    def forward(self, hidden_states, attention_mask, sample_embed_dim=-1, num_attention_head=-1, qkv_size=-1,
        ...
        attention_scores = attention_scores / \
            math.sqrt(self.sample_attention_head_size)
        ...
        context_layer = torch.matmul(
            self.dropout(attention_probs), value_layer)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[
            :-2] + (self.sample_qkv_size,)
        context_layer = context_layer.view(*new_context_layer_shape)# 多头注意力输出的结果进行拼接
        return context_layer, attention_scores #attention_scores是在应用softmax之前的结果
...
class SuperBertSelfOutput(nn.Module):
    ...
    def set_sample_config(self, qkv_size, sample_embed_dim, in_index=None):
        self.dense.set_sample_config(
            qkv_size, sample_embed_dim, in_index=in_index)
        self.LayerNorm.set_sample_config(sample_embed_dim)
...
class SuperBertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, sample_embed_dim=-1,
        ...
        return attention_output, layer_att #attention_output是hidden state，layer_att是自注意力在softmax之前的结果
...
class SuperBertLayer(nn.Module):
    ...
    def set_sample_config(self, sample_embed_dim, intermediate_size, num_attention_head, qkv_size):
        ...
        self.intermediate.set_sample_config(
            sample_embed_dim, intermediate_size)
    ...
    def forward(self, hidden_states, attention_mask, sample_embed_dim=-1,
        ...
        attention_output = self.attention(hidden_states, attention_mask,
                                          sample_embed_dim, num_attention_head, qkv_size,
                                          in_index=in_index, out_index=out_index)
        attention_output, layer_att = attention_output#attention_output是hidden state，layer_att是自注意力在softmax之前的结果
        ...
        return layer_output, layer_att #layer_output是一层encoder之后的hidden state，layer_att是自注意力在softmax之前的结果
...
class SuperBertEncoder(nn.Module):
    ...
    def set_sample_config(self, subbert_config):
        ...
        for layer, num_attention_head, intermediate_size, qkv_size in zip(self.layers[:self.sample_layer_num],
            ...
            layer.set_sample_config(
                sample_embed_dim, intermediate_size, num_attention_head, qkv_size)
    ...
    def forward(self, hidden_states, attention_mask, subbert_config=None, kd=False,
        ...
        for i, layer_module in enumerate(self.layers[:sample_layer_num]):
            ...
            hidden_states, layer_att = hidden_states#hidden_states是一层encoder之后的hidden state，layer_att是自注意力在softmax之前的结果
            all_encoder_att.append(layer_att)

        all_encoder_layers.append(hidden_states)# all_encoder_layers length=13, all_encoder_att length=12
...
class SuperBertPooler(nn.Module):
    ...
    def forward(self, hidden_states, sample_hidden_dim):
        ...
        first_token_tensor = hidden_states[:, 0]#取出[CLS]对应的hidden state
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_scratch(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if not os.path.exists(resolved_config_file):
            resolved_config_file = os.path.join(
                pretrained_model_name_or_path, BERT_CONFIG_NAME)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if not os.path.exists(config_file):
            # Backward compatibility with old naming format ！老命名方式是bert_config.json
        ...
        for old_key, new_key in zip(old_keys, new_keys):
            state_dict[new_key] = state_dict.pop(old_key)
        # ！由于pre-train的结构与目前定义的结构在命名上有区别，需要进行修改
        old_keys = []
        new_keys = []
        for key in state_dict.keys():
            new_key = None
            if key.startswith('bert.embeddings'):
                new_key=key.split('.')
                if new_key[2]!='LayerNorm':
                    new_key.insert(3,'embedding')
...
class SuperBertModel(BertPreTrainedModel):
    def __init__(self, config, fit_size=768):
        ...
        self.dense_fit = SuperLinear(config.hidden_size, fit_size)# ！这个地方注意一下是否使用
    ...
    def calc_sampled_param_num(self):
        ...
        logger.info('all parameters: {}\n'.format(
            emb_numel + encoder_numel + pooler_numel))
        logger.info('===========================')
    ...
    def forward(self, input_ids, subbert_config,
        ...
        if kd:
            ...
            for i in range(head_number):
                ...
                in_out_index.extend(
                    range(start_ind, start_ind + in_sample_per_head))
        ...
        if kd:
            ...
            self.dense_fit.set_sample_config(
                subbert_config['sample_hidden_size'], self.fit_size)
            ...
            if not kd_infer:
                ...
                pooled_output = self.pooler(
                    last_rep, subbert_config['sample_hidden_size'])
            ...
            sequence_output = all_encoder_layers[-1]# sequence_output是最后一层输出的hidden state
            pooled_output = self.pooler(
                sequence_output, subbert_config['sample_hidden_size'])
            return sequence_output, pooled_output# pooled_output是进过pooler层的输出，对于句子级分类都有这个处理，然后交给后边的classifier
...
class SuperBertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.predictions = SuperBertLMPredictionHead(
    ...
    def forward(self, sequence_output, pooled_output, sample_hidden_dim):
        ...
        prediction_scores = self.predictions(
            sequence_output, sample_hidden_dim)
...
class SuperBertForPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = SuperBertModel(config)
        self.cls = SuperBertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.embedding.weight)
        self.apply(self.init_bert_weights)
        self.config = config
    ...
    def forward(self, input_ids, subbert_config, token_type_ids=None, attention_mask=None,
        ...
        encoder_layers, pooled_output = self.bert(
            input_ids, subbert_config, token_type_ids, attention_mask)
...
class SuperBertForSequenceClassification(BertPreTrainedModel):
    ...
    def set_sample_config(self, subbert_config):
        ...
        self.classifier.set_sample_config(
            subbert_config['sample_hidden_size'], self.num_labels)
...
class SuperBertForQuestionAnswering(BertPreTrainedModel):
    ...
    def set_sample_config(self, subbert_config):
        self.bert.set_sample_config(subbert_config)
        self.qa_outputs.set_sample_config(
            subbert_config['sample_hidden_size'], 2)
    ...
    def forward(self, input_ids, subbert_config, attention_mask=None, token_type_ids=None,
        ...
        if not kd_infer:
            encoded_layers, pooled_output = self.bert(input_ids, subbert_config,
                                                      ...
                                                      token_type_ids=token_type_ids,
                                                      kd=False, kd_infer=False)
            # last_sequence_output = encoded_layers[-1]# ?这里有问题
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_positions.clamp_(0, ignored_index)# 将start_position限制在[0,start_logits.size[1]],同时start_logits.size[1]也是ignored_index
        ...
        return logits# 对于squad问题，dev数据start_positions=None,end_positions=None，此时返回logits
