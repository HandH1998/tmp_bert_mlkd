...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs, reduce_memory=False):
        ...
        metrics_file = training_path / \
            "epoch_{}_metrics.json".format(self.data_epoch)
...
def main():
    ...
    for i in range(int(args.num_train_epochs)):
        ...
        metrics_file = args.pregenerated_data / \
            "epoch_{}_metrics.json".format(i)
        if epoch_file.is_file() and metrics_file.is_file():
            ...
            print("Warning! There are fewer epochs of pregenerated data ({}) than training epochs ({}).".format(
    ...
    tensorboard_writer = SummaryWriter(log_dir=args.tensorboard_log_dir, flush_secs=30)
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        with tqdm(total=len(train_dataloader), desc="Epoch {}".format(epoch)) as pbar:
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                student_atts, student_reps, student_att_probs, student_all_self_outs, original_student_reps, student_words_embeddings = student_model(
                    input_ids, segment_ids, input_mask)
                teacher_reps, teacher_atts, _, _, teacher_all_self_outs, teacher_words_embeddings = teacher_model(
                    input_ids, segment_ids, input_mask)
                ...
                # teacher_reps = [teacher_rep.detach()
                #                 for teacher_rep in teacher_reps]  # speedup 1.5x
                # teacher_atts = [teacher_att.detach()
                #                 for teacher_att in teacher_atts]
                ...
                mean_rkd_emb_loss = tr_rkd_emb_loss * args.gradient_accumulation_steps / nb_tr_steps
                mean_rep_loss = tr_rep_loss * args.gradient_accumulation_steps / nb_tr_steps
                mean_self_out_loss = tr_self_out_loss * args.gradient_accumulation_steps / nb_tr_steps
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if (global_step + 1) % args.eval_step == 0:
                        ...
                        result['rkd_emb_loss'] = mean_rkd_emb_loss
                        result['rep_loss'] = mean_rep_loss
                        result['self_out_loss'] = mean_self_out_loss
                        output_eval_file = os.path.join(
                            args.output_dir, "log.txt")
                        tensorboard_writer.add_scalar('loss', mean_loss, global_step)
                        tensorboard_writer.add_scalar('rkd_emb_loss', mean_rkd_emb_loss, global_step)
                        tensorboard_writer.add_scalar('rep_loss', mean_rep_loss, global_step)
                        tensorboard_writer.add_scalar('self_out_loss', mean_self_out_loss, global_step)
